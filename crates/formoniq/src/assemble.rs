use crate::operators::{
  ...
  InnerProductWeightClosure, Nc1LumpedMassElmat, Nc1MassElmat, Nc2LumpedMassElmat, Nc2MassElmat,
};
...
fn nc2_global_dof(face_kidx: usize, slot: usize) -> usize {
  3 * face_kidx + slot
}
...
fn nc2_vertex_dofs(topology: &Complex) -> Vec<Vec<usize>> {
  assert_supported_nc2_dim(topology.dim());

  let mut vertex_dofs = vec![Vec::new(); topology.vertices().len()];
  for face in topology.facets().handle_iter() {
    for slot in 0..3 {
      vertex_dofs[face[slot]].push(nc2_global_dof(face.kidx(), slot));
    }
  }
  for dofs in &mut vertex_dofs {
    dofs.sort_unstable();
...
fn nc2_lumped_mass_inverse_blocks(
  topology: &Complex,
  lumped_mass: &GalMat,
) -> (Vec<Vec<usize>>, Vec<Matrix>) {
  assert_supported_nc2_dim(topology.dim());
  let ndofs = 3 * topology.facets().len();
  assert_eq!(lumped_mass.nrows(), ndofs);
  assert_eq!(lumped_mass.ncols(), ndofs);

  let vertex_dofs = nc2_vertex_dofs(topology);
  let mut dof_locations = vec![None; ndofs];
  let mut blocks = Vec::with_capacity(vertex_dofs.len());
  for (ivertex, dofs) in vertex_dofs.iter().enumerate() {
    for (ilocal, &dof) in dofs.iter().enumerate() {
  ...
  for (r, c, &v) in lumped_mass.triplet_iter() {
    let Some((ivertex_r, ilocal_r)) = dof_locations[r] else {
      panic!("NC2 dof {r} is not associated with a mesh vertex.");
    };
    let Some((ivertex_c, ilocal_c)) = dof_locations[c] else {
      panic!("NC2 dof {c} is not associated with a mesh vertex.");
    };

    if ivertex_r != ivertex_c {
      assert!(
        v.abs() <= 1e-12,
        "NC2 lumped mass must not couple dofs attached to different vertices."
      );
      continue;
    }
    blocks[ivertex_r][(ilocal_r, ilocal_c)] += v;
  ...
  let inverse_blocks = blocks
    ...
    .map(|block| {
      block
        .clone()
        .cholesky()
        .expect("NC2 lumped mass vertex block must be positive definite.")
        .inverse()
    })
    .collect();
...
fn assemble_nc2_lumped_mass_inverse_from_blocks(
  ndofs: usize,
  vertex_dofs: &[Vec<usize>],
  inverse_blocks: &[Matrix],
) -> GalMat {
...
fn assemble_whitney_2form_projected_sparse_inverse_from_blocks(
  topology: &Complex,
  vertex_dofs: &[Vec<usize>],
  inverse_blocks: &[Matrix],
) -> GalMat {
  let nfaces = topology.facets().len();
  let mut galmat = GalMat::new(nfaces, nfaces);
  for (dofs, block) in vertex_dofs.iter().zip(inverse_blocks) {
    for (ilocal, &idof) in dofs.iter().enumerate() {
      let iface = idof / 3;
      for (jlocal, &jdof) in dofs.iter().enumerate() {
        let jface = jdof / 3;
        let value = (1.0 / 9.0) * block[(ilocal, jlocal)];
        if value != 0.0 {
          galmat.push(iface, jface, value);
        }
      }
    }
  }
  ...
  M: std::ops::Index<(usize, usize), Output = f64> + Send,
{
  ...
  let triplets: Vec<(usize, usize, f64)> = topology
    ...
    .flat_map(|cell| {
      ...
      let local_faces: Vec<_> = cell.mesh_subsimps(2).collect();
      let nlocal_dofs = 3 * local_faces.len();

      let mut local_triplets = Vec::new();
      for ilocal in 0..nlocal_dofs {
        let iglobal = nc2_global_dof(local_faces[ilocal / 3].kidx(), ilocal % 3);

        for jlocal in 0..nlocal_dofs {
          let jglobal = nc2_global_dof(local_faces[jlocal / 3].kidx(), jlocal % 3);
          let val = elmat[(ilocal, jlocal)];
          if val != 0.0 {
            local_triplets.push((iglobal, jglobal, val));
          }
...
pub fn assemble_nc2_mass_galmat(topology: &Complex, geometry: &MeshLengths) -> GalMat {
  let dim = topology.dim();
  assemble_nc2_galmat_impl(topology, geometry, move |geo, _cell| {
  ...
  T: AddAssign + Mul<f64, Output = T> + ApplyWeight,
{
  ...
  assemble_nc2_galmat_impl(topology, geometry, move |geo, cell| {
    elmat.eval_with_coords(geo, cell)
  })
}
...
pub fn assemble_nc2_lumped_mass_galmat(topology: &Complex, geometry: &MeshLengths) -> GalMat {
  let dim = topology.dim();
  assemble_nc2_galmat_impl(topology, geometry, move |geo, _cell| {
    Nc2LumpedMassElmat::new(dim).eval(geo)
  })
}
...
pub fn assemble_nc2_lumped_mass_galmat_weighted(
  ...
  let dim = topology.dim();
  let elmat = Nc2LumpedMassElmat::new_weighted(dim, coords, qr, weight);
  assemble_nc2_galmat_impl(topology, geometry, move |geo, cell| {
    elmat.eval_with_coords(geo, cell)
  })
}
...
pub fn assemble_nc2_lumped_mass_inverse_galmat(
  ...
  let lumped_mass = assemble_nc2_lumped_mass_galmat(topology, geometry);
  let (vertex_dofs, inverse_blocks) = nc2_lumped_mass_inverse_blocks(topology, &lumped_mass);
  assemble_nc2_lumped_mass_inverse_from_blocks(lumped_mass.nrows(), &vertex_dofs, &inverse_blocks)
}
...
pub fn assemble_nc2_lumped_mass_inverse_galmat_weighted(
  ...
  let lumped_mass =
    assemble_nc2_lumped_mass_galmat_weighted(topology, geometry, coords, qr, weight);
  let (vertex_dofs, inverse_blocks) = nc2_lumped_mass_inverse_blocks(topology, &lumped_mass);
  assemble_nc2_lumped_mass_inverse_from_blocks(lumped_mass.nrows(), &vertex_dofs, &inverse_blocks)
}
...
pub fn assemble_nc2_to_whitney_projection_galmat(topology: &Complex) -> GalMat {
  ...
  let nfaces = topology.facets().len();
  let mut galmat = GalMat::new(nfaces, 3 * nfaces);
  for iface in 0..nfaces {
    for slot in 0..3 {
      galmat.push(iface, nc2_global_dof(iface, slot), 1.0 / 3.0);
    }
  }
  galmat
}
...
pub fn assemble_whitney_to_nc2_embedding_galmat(topology: &Complex) -> GalMat {
  ...
  let nfaces = topology.facets().len();
  let mut galmat = GalMat::new(3 * nfaces, nfaces);
  for iface in 0..nfaces {
    for slot in 0..3 {
      galmat.push(nc2_global_dof(iface, slot), iface, 1.0);
    }
  }
  galmat
}
...
pub fn assemble_whitney_2form_projected_sparse_inverse_galmat(
  ...
  let lumped_mass = assemble_nc2_lumped_mass_galmat(topology, geometry);
  let (vertex_dofs, inverse_blocks) = nc2_lumped_mass_inverse_blocks(topology, &lumped_mass);
  assemble_whitney_2form_projected_sparse_inverse_from_blocks(
    topology,
    &vertex_dofs,
    &inverse_blocks,
  )
