...
use {
  ...
  formoniq::{
    assemble::{self, assemble_galvec, assemble_whitney_projected_sparse_inverse_galmat},
...
const DIM: usize = 3;
const GRADE: usize = 2;
const DEFAULT_MAX_REFINEMENT: u32 = 4;
const KAPPA: f64 = 0.0;
const OUTPUT_DIR: &str = "out/examples/hodge_laplace_2form_nc1_lumping";
const SOLUTION_WEIGHTS: [f64; 3] = [1.0, 2.0, -1.0];

fn max_refinement() -> u32 {
  std::env::var("FORMONIQ_MAX_REFINEMENT")
    .ok()
    .and_then(|value| value.parse().ok())
    .unwrap_or(DEFAULT_MAX_REFINEMENT)
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  for refine in 0..=max_refinement() {
    ...
    println!(
      "| {:>2} | {:>5} | {:>10.2e} | {:>6.2} | {:>10.2e} | {:>6.2} | {:>10.2e} | {:>6.2} | {:>10.2e} | {:>6.2} | {:>10.2e} | {:>6.2} | {:>10.2e} | {:>6.2} | {:>10.2e} | {:>10.2e} |",
      row.refine,
      row.nboxes_per_dim,
      row.mixed_l2,
      row.mixed_l2_rate,
  ...
  writeln!(
    summary,
    "The mixed solve uses sparse LU on the reduced system. The nc1 path uses a reduced projected-NC1 Schur operator. The rsum path uses a reduced Schur operator with row-sum lumping of the consistent (k-1)-mass. Both operator paths use sparse Cholesky."
  )
  .unwrap();
  writeln!(
    summary,
    "Default max refinement is {}. Override with FORMONIQ_MAX_REFINEMENT to trade memory for more levels.",
    max_refinement()
  )
  .unwrap();
  ...
  fs::write(format!("{OUTPUT_DIR}/summary.txt"), summary)?;
