...
fn main() {
  ...
  let grad_exact_solution = DiffFormClosure::one_form(
    |p| {
      Vector::from_iterator(
        p.len(),
        [PI * (PI * p[0]).cos() * p[1], (PI * p[0]).sin()].into_iter(),
      )
    },
    dim,
  );
  ...
  let neumann_dof_selector =
    |sidx: manifold::topology::handle::SimplexIdx| neumann_dofs.contains(&sidx);
